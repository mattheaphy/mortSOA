...
The
[`read_mort_soa()`](https://mattheaphy.github.io/mortSOA/reference/read_mort_soa.md)
function is used to obtain data from `mort.soa.org`. This function
requires a table identification number (`table_id`). Continuing with the
prior example, let’s look up table 2582, the 2012 IAM Basic Tables for
female lives.
...
using the [`attr()`](https://rdrr.io/r/base/attr.html) or
[`attributes()`](https://rdrr.io/r/base/attributes.html) functions.
...
these tables, the list returned by
[`read_mort_soa()`](https://mattheaphy.github.io/mortSOA/reference/read_mort_soa.md)
will include two elements. The first element is a two-dimensional table
with mortality rates varying by issue age and policy duration. The
second element is an ultimate mortality table varying by attained age
only.
